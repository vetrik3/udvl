...
class Case {
    ...
    Case(Map<String, Boolean> interpretation, boolean result) {
...
class Tester {
    ...
    public void test(Formula formula, String string, Case[] cases) {
        ...
        for (Case c : cases) {
            System.out.println(" interpretation " + c.interpretation + ":");
            compare(formula.eval(c.interpretation), c.result, "eval");
        }
    }
    
    public void status() {
        System.out.println("TESTED " + tested + "PASSED " + passed);
        if (tested == passed) {
...
public class Cv03 {
    ...
    public static void main(String[] args) {
        ...
        Map<String,Boolean> i1 = new HashMap<>();
        i1.put("a", true);
        
        Map<String,Boolean> i2 = new HashMap<>();
        i2.put("a", false);
        
        t.test(new Variable("a"), "a",new Case[] {
            new Case(i1, true), new Case(i2, false)
        });

        t.test(new Negation(new Variable("a")), "-a", new Case[] {
           new Case(i1, false), new Case(i2, true) 
        });
